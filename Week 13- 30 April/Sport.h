...
void Sport::setAllSportData()
{
    ...
    cout << "\nCreate a New Sport Entry" << std::endl;
...
void Sport::displaySport() const
{
    ...
    cout << "Sport Name: " << sportName << std::endl;

    //GameDate Block
    if (next.day == 0 && next.month == 0 && next.year == 0)
    {
        cout << "Date of next game: <empty>" << std::endl;
    }
    else
    {
        cout << "Date of next game: " << next.day << "/" << next.month << "/" << next.year << std::endl;
    }

    //Teams Block
    if (teams == 0)
    {
        cout << "Number of Teams: <empty>" << std::endl;
    }
    else
    {
        cout << "Number of Teams: " << teams << std::endl;
    }
}
